...
schmittTrig_v3<- function(dataframe, time, dFF, normIntensity, normBaseline, outlier_indices, threshold, std, ymax, ymin, thresh_stdev) {
    ...
    if (showGraphs == TRUE) {
    par(mfrow = c(4,1),oma = c(2,2,0,0) + 0.1,mar = c(3.7,3.7,1,1) + 0.2) #tiling 3 by 1 plots
