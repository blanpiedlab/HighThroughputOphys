...
schmittTrig<- function(dataframe, time, dFF, normIntensity, normBaseline, normStDev, threshold, std, thresh_stdev) {
    ...
    if (showGraphs == TRUE) {
    par(mfrow = c(3,1),oma = c(2,2,0,0) + 0.1,mar = c(3.7,3.7,1,1) + 0.2) #tiling 3 by 1 plots
    
    #normIntensity vs time with normBaseline overlay
    plot(time,normIntensity, main = 'Normalized intensity signal with baseline', xlab="",col = "black", type = "l")    
    lines(time,normBaseline, col = "red")
    lines(time,(normBaseline+(normStDev*thresh_stdev)), type='l',col = 'green', lwd=2)
    lines(time,(normBaseline-(normStDev*thresh_stdev)), type='l',col = 'green', lwd=2)
    
    
    #Show dFF vs time with Schmitt Trigger levels
    plot(time,dFF,type="l",main = "dFF with peaks detected", xlab="",col="black",lwd=1)
