...
#dir = setwd("Y:\\Sam/2022-09-08/datasets/fullPP")
						...
						groupers =c("sensor",'transduction',"segmentation","neuronSegment","dish","plate", "region", "Ca", "protocol", "exposeNum", "ROINumber") 
						#segmentationMethod = "activity"
						#TTL_start = 199
						#read data - is ready for primetime with whatever data 
						source("Z:\\R Scripts/Sam current as of 7.28.22/Active scripts/pipeline v5 - generalized/peakFinder/read_csv_filename_infectionExp.R")
						# groupers for the first set of scripts - this shouldn't change much experiment to experiment
						groupers =c("sensor",'transduction',"segmentation","neuronSegment","dish","plate", "region", "Ca", "protocol", "exposeNum", "ROINumber") 

						#peak identification
						showGraphs = FALSE 																			# set to TRUE or FALSE to see the graph outputs from peakFinder_Heavy.R/peakFinder_Lite.R
						...
						source("Z:\\R Scripts/Sam current as of 7.28.22/Active scripts/pipeline v5 - generalized/peakFinder/flagOutliers.R") 
						source("Z:\\R Scripts/Sam current as of 7.28.22/Active scripts/pipeline v5 - generalized/peakFinder/peakFinder_Heavy.R") 

						## can write in a variance filter a la "peaks that pass"
						## probably groupers works, but working in ROI_keys might be more elegant
						source("Z:\\R Scripts/Sam current as of 7.28.22/Active scripts/pipeline v5 - generalized/peakFinder/peakFinder_Lite.R") 
						
						
						source("Z:\\R Scripts/Sam current as of 7.28.22/Active scripts/pipeline v5 - generalized/peakFinder/annotated_csv_output.R") 
