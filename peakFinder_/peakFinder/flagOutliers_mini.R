...
k_duration_normal = 0.05 #s
k_duration_long = 0.25 #s
...
if (length(dataset_id) > 1) {
	...
	k=ceiling(k_duration_long/interFrame)
	window = ceiling(minimum_window_duration/interFrame)
...
data_flagged <- suppressMessages(left_join(data,getTrace.stdev) %>%
      ...
      mutate(intensity_smooth = rollapply(intensity, k, mean, fill=NA, align="center"),
...
rm(list=ls()[! ls() %in% c("data_flagged","groupers",'showGraphs','mainDir','n','savePlots')])  #clear all vars in memory except for flagged data.
