...
plot_peakStats<- function(peak_stat_df = peak_stats, df = df, groupers = groupers, color_override = color_override, remove_cols=drops, plotBy = plotBy, levels=levels,remove_col = drops){
                        ...
                        remove_neurons = c("activity-dish05-plate03-region02", "activity-dish05-plate06-region01", "activity-dish05-plate06-region02",
                        		   "marker-dish05-plate03-region02", "marker-dish05-plate06-region01", "marker-dish05-plate06-region02")#,"dish05-plate01-region01")
                        

                        peak_stat_df <- peak_stat_df %>% mutate(windowedPeakID = paste0("windowed",peakID),
                        				 imaging_region = gsub("-repl\\d\\d", "", vid_key),
                        				 #imaging_region = gsub("activity-|marker-","",seg_imaging_region) 
                        				 ) %>%
...
###### for loop for plotting spatial data
        png_dir = "Y:\\Sam/paper1_datasets/singleAP_v2/imageFiles/zstack/zstack_figure2_demo"
        ...
        for (k in 1:length(vid_key_vec)){
		   ...
		   assign(nam, tmp_plot,envir = .GlobalEnv)
                ...
                for(i in 1:length(Ca_vec)) {
                					...
                					if(current_Ca == "0.5Ca") {activity_map = "0pt5Ca_ROImap.zip"} 
                					if(current_Ca == "1Ca") { activity_map = "1Ca_ROImap.zip"}
                					if(current_Ca == "2Ca") { activity_map = "2Ca_ROImap.zip"}
                					if(current_Ca == "4Ca") { activity_map = "4Ca_ROImap.zip"}
                                    ...
                                    if(save_bool==TRUE) { 
                                       ...
                                       tmp_plot = png_plotter_ROIoverlay(df = Ca_df, 
                                       									png_dir = png_dir,
 ...
 ####### plot cumdists
                        ...
                        xmax = max(peak_stat_df$interSpike_ms)
                         

                         ecdf_interspike<-ggplot(peak_stat_df, aes(x=interSpike_ms, group=segmentation, linetype=segmentation,colour=Ca))+
                                             stat_ecdf(size=2,alpha=1, lineend="round")+
                                             
                                            labs( x=expression(Delta*"t (ms)"),
                                                    y="Cumulative Density",
                                                    title=expression(Delta*"t (ms)")
                                                    )+
                                            ...
                                            {if(color_switch)scale_colour_manual(labels=new_levels, values=color_override, guide= "none")}+
                                            scale_linetype_manual(name = "", values = c("activity" = "dashed", 'marker' = 'solid'))+
                                            coord_cartesian(ylim=c(0,1.1),xlim=c(0,150))+
                                            scale_y_continuous(breaks=c(0,0.5,1.0))+
                                            #scale_x_continuous(breaks=c(60,75,100,150,500), labels = remove_labels(scales::label_number_auto(), c(60,100)), guide=guide_axis(angle=45))+
                        ...
                        ggsave(filename=paste0("interSpike_ms_cumdist.jpeg"),plot=ecdf_interspike, device="jpeg",dpi=600, bg="white",units="in",width=cumdist_width,height=cumdist_height)
...
### as boxplots
    				violin_amplitude<-ggplot(peak_stat_df, aes(x=segmentation, y=amplitude, group=segmentation, colour=segmentation))+#linetype=segmentation,
                        ...
                        violin_interSpike_ms<-ggplot(peak_stat_df, aes(x=segmentation, y=interSpike_ms, group=segmentation, colour=segmentation))+#linetype=segmentation,
                                             #stat_ecdf(size=2,alpha=1, lineend="round")+
                                             geom_sina(alpha=0.3)+
                                             geom_violin(fill=NA,draw_quantiles = c(0.25, 0.5, 0.75))+
                                            labs( y=expression(Delta*"t (ms)"),
                                                    x="",#"Cumulative Density",
                                                    title=expression(Delta*"t (ms)"),
                                                    colour="Segmented by:"
                                                    )+
                        ...
                        ggsave(filename=paste0("interSpike_ms_violin.jpeg"),plot=violin_interSpike_ms, device="jpeg",dpi=600, bg="white",units="in",width=cumdist_height,height=cumdist_width)
...
##### HISTOGRAMS
					   ...
					   histo_amplitude<-ggplot(peak_stat_df, aes(x=amplitude, group=segmentation, fill=segmentation,colour=segmentation))+
					   					   geom_vline(data=my_medians, aes(xintercept = median_amplitude, colour=segmentation),size = scalefactor * 2, lty = "solid",show_guide = FALSE,alpha=0.9) +
    										   ...
    										   geom_histogram(aes(y = ..ncount..,alpha=segmentation), alpha=0.4,binwidth = bin, boundary = 0, position = "identity", colour = "black", size = scalefactor * 0.5,show_guide = TRUE) +
                         ...
                         bin = (upper_bound_x - lower_bound_x) / 50  
					      ...
					      histo_interSpike_ms<-ggplot(peak_stat_df, aes(x=interSpike_ms, group=segmentation, fill=segmentation,colour=segmentation))+
					                                         geom_vline(data=my_medians, aes(xintercept = median_interSpike, colour=segmentation),size = scalefactor * 2, lty = "solid",show_guide = FALSE,alpha=0.9) +
    										   geom_freqpoly(aes(y = ..ncount..), binwidth = bin, alpha = 0.8, boundary = 0, size = scalefactor * 1,show_guide = FALSE) +
    										   geom_histogram(aes(y = ..ncount..,alpha=segmentation), alpha=0.4,binwidth = bin, boundary = 0, position = "identity", colour = "black", size = scalefactor * 0.5,show_guide = TRUE) +
    										#     geom_vline(data=my_medians, aes(xintercept = median_interSpike, colour=segmentation), size=scalefactor*2, lty="solid" )+
					                                        ...
					                                        #     geom_histogram(aes(y=..ncount..,alpha=segmentation),binwidth = bin,alpha=0.4, boundary = 0,position="identity",colour="black",size=scalefactor*0.5)+
					                                            ...
					                                            coord_cartesian(ylim=c(0,1.3),xlim=c(0,150))+
					                                            scale_y_continuous(breaks=c(0,0.5,1.0), expand=c(0,0))+
					                                            #scale_x_continuous(breaks=c(60,75,100,150,500), labels = remove_labels(scales::label_number_auto(), c(60,100)), guide=guide_axis(angle=45))+
					     ...
					     ggsave(filename=paste0("histo_interSpike_ms.jpeg"),plot=histo_interSpike_ms, device="jpeg",dpi=600, bg="white",units="in",width=cumdist_width,height=cumdist_height)

					     dt_histo <<- histo_interSpike_ms
					     ...
					     histo_t_half<-ggplot(cleanup_time_data, aes(x=t_half, group=segmentation, fill=segmentation,colour=segmentation))+
					                                            geom_vline(data=time_data_medians, aes(xintercept = median_t_half, colour=segmentation), size=scalefactor*2, lty="solid" )+
...
#                             stat_summary_bin(aes(group=Ca), geom="smooth", fun=mean, binwidth=interFrame.var,size=3)+
