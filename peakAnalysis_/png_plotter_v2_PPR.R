...
png_plotter<- function(df = df, png_dir = png_dir,prefix_str=prefix_str, suffix_str = suffix_str, dir_regex = dir_regex, vars_to_plot=vars_to_plot,save_bool = save_bool,guide_bool=guide_bool,tag_var=tmp_tag_var, ROI_IDs = ROInames){
											 ...
											 for(k in 1:length(vars_to_plot)) {
											 	...
											 	if(guide_bool == TRUE) {
														 	p2<- ggplot() +
															  ...
															  scale_fill_gradient(low="blue",high="orange",na.value=na.value.forplot,limits=c(0.5,2.5),name="PPR", oob=scales::squish)+
															  scale_colour_gradient(low="blue",high="orange",na.value=na.value.forplot,limits=c(0.5,2.5), oob=scales::squish,guide="none")+
															  ...
															  scale_x_continuous(expand=c(0,0),limits=c(0,upper_range_x), oob=scales::oob_keep)+
															  my.theme_spatialROI+
															  guides(fill=guide_legend(title="PPR",override.aes = list(size = 1.2)))+
