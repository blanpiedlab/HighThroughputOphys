...
subDir <- "transmission_stats_t4_emptyPNG"   
...
source("Z:\\R Scripts/Sam current as of 7.28.22/Active scripts/pipeline v5 - generalized/peakAnalysis/png_plotter_v2_synTrans.R")
...
#### having generated a summary dataframe that keeps all of the sensor_trackROI_keys that feature at least 1 event, we can now do estimates of frequency on the basis of chemical_condition (control or vehicle/APV)
                ...
                clean_freq_vehicle <- clean_freq %>% dplyr::filter(trackROI_key %in% clean_vehicle_ROI_list) %>% mutate(groupID = paste(sensor,"Vehicle",sep='-') )
                
                clean_APV_ROIs<- clean_freq %>% dplyr::filter(chemical_condition == "APV") 
                clean_APV_ROI_list<- unique(clean_APV_ROIs$trackROI_key)
                clean_freq_APV <- clean_freq %>% dplyr::filter(trackROI_key %in% clean_APV_ROI_list) %>% mutate(groupID = paste(sensor,"AP5",sep='-') )
...
# ### GENERATE A PLOT OF PEAK FREQUENCY AS A FUNCTION OF EXCHANGE
                    ...
                    levels_groupID = c('GluSnFR3-Vehicle','GluSnFR3-AP5','JF646-Vehicle','JF646-AP5')
...
#                                                   tag="J")+
#                                            scale_colour_manual(name = "", values = c("GluSnFR3-Vehicle" = "#339900", 'GluSnFR3-AP5' = '#669900', "JF646-Vehicle" ="#FF33FF","JF646-AP5" ="#993366"))+
#                                            scale_shape_manual(name = "", values = c("GluSnFR3-Vehicle" = 16, 'GluSnFR3-AP5' = 21, "JF646-Vehicle" =16,"JF646-AP5" = 21))+
#                                            scale_linetype_manual(name = "", values = c("GluSnFR3-Vehicle" = "solid", 'GluSnFR3-AP5' = 'longdash', "JF646-Vehicle" ="solid","JF646-AP5" ="longdash"))+
#                                            scale_alpha_manual(name="",values = c("GluSnFR3-Vehicle" = 1.0, "GluSnFR3-AP5" = 0.8, "JF646-Vehicle" = 1.0, "JF646-AP5" =0.8))+
                                           
#                                             coord_cartesian(xlim=c(0.9,2.1),ylim=c(0,0.2))+
...
#### save to global env
                    ...
                    clean_transmission_summary<- clean_transmission_analysis %>% mutate(isTransmission_fixed = case_when(isTransmission == TRUE ~ "success",
                                                                                ...
                                                                                summarise(n_success = n[which(isTransmission_fixed=="success")],
...
#             ggsave(filename=paste0("basePlot_transmissionPlot.png"),plot=transmissionPlot, device="png",dpi=600, units="in",width=lineplot_width,height=plot_height)
                    ...
                    #                        stat_summary(aes(group=groupID, colour=groupID,shape=groupID), geom="point", stroke=3,fill='white',fun=mean,size=5,alpha=1,na.rm=TRUE)+
                    #                        labs( x="",
                    #                               y=expression(P[transmission]),
                    #                               ##title="",
                    #                               #subtitle="Averaged response from three trials per protocol",
                    #                               tag="")+
                    #                        scale_colour_manual(name = "", values = c("Vehicle" = "black", 'AP5' = 'black'))+
                    ...
                    #                         save_plot(filename=paste0("transmission_rate_v1"),plot=transmission_rate_v1, plot_width=plot_dim*0.5,plot_height=plot_dim, scale_f = scalefactor,dpi_val = 600)
...
#                                 histo_success_D  <<- amplitude_histogram
...
# ###### for loop for plotting spatial data
        png_dirs = c("Y:\\Sam/paper1_datasets/synapticTransmission_v1/APV/Figures/synaptic_transmission_v4/synTransmission_v1/forMapping/_registered_3","Y:\\Sam/paper1_datasets/synapticTransmission_v1/APV/Figures/synaptic_transmission_v4/synTransmission_v1/forMapping/_registered_6","Y:\\Sam/paper1_datasets/synapticTransmission_v1/APV/Figures/synaptic_transmission_v4/synTransmission_v1/forMapping/_registered_11")
