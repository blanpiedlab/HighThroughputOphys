...
png_plotter_ROIoverlay<- function(df = df, png_dir = png_dir,prefix_str=prefix_str, suffix_str = suffix_str, dir_regex = dir_regex, vars_to_plot=vars_to_plot,save_bool=save_bool,tag_var=taggy, ROI_IDs = ROInames,user_x = user_x, user_y = user_y){
				...
				######read_png############ so you can change directory to read in Ch2Results.csv
							 ...
							 scalebar = data.frame(x_start = user_x,
							  ...
							  upper_range_y = 256/pixel_to_um#ifelse(range(roi_polygons$y_pixel)[2] > 256, range(roi_polygons$y_pixel)[2], 256)/pixel_to_um 
								  ...
								  labs(x = expression("X ("*mu*"m)"), y = expression("Y ("*mu*"m)"), title = "ROI Overlay",tag=tag_var)+#paste0(fixed_vid_key,": Spatial Plot with ROI overlay only")) +
								  theme_minimal()+
								  ...
								  if(save_bool == TRUE) {
									...
									ggsave(filename=paste0(fixed_vid_key,"_spatial_plot_onlyROIs_overlay.png"),plot=p1, device="png",dpi=600, bg="white",units="in",width=16,height=16)
								}
...
# 		p = png_plotter(df=check[i],png_dir=png_dir,prefix_str=prefix_str,suffix_str=suffix_str,dir_regex=dir_regex)
# 		print(p)
# }                                          
...
#trying geom_sf stuff


# 	#do shapefile stuff from https://www.paulamoraga.com/book-spatial/the-sf-package-for-spatial-vector-data.html and https://github.com/r-spatial/sf/issues/1233
# 	
# 	#out_poly<-st_polygon(list(ROIcoords))
# 	#out<-data.frame(ROIcoords)
# 	out_owin = owin(poly=out)
# 	W_sfc<- st_as_sfc(out_owin)
# 	ROINumber=paste0("ROI",str_pad(i, 4, pad= "0"))
# 	p_sf<-st_sf(ROINumber=ROINumber, geometry=W_sfc)
# 	out_sf<- p_sf %>% as.data.frame()
# 	
# 	#ROI_spat = rbind(ROI_spat, out)
# 	#demonstrate it is working on per ROI basis
# 	#g<-ggplot(p_sf) + geom_sf(aes(col = ROINumber), size = 3) + theme_bw()
# 	#print(g)
# 	ROI_sf_combined<- rbind(ROI_sf_combined, out_sf)
# 	#print(out)
# 	#print(p_sf)
# #	print(paste0("finished executing loop: ", i ) ) 
# 	
# }
#g<-ggplot(ROI_sf_combined) + 
#  annotation_raster(y, xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf)+
#  
#    geom_sf(aes(geometry=geometry,col = ROINumber), size = 3, fill=NA) + 
#  theme_bw() + 
#  coord_sf(xlim=c(0,256),ylim=c(256,0),expand=TRUE)
#print(g)

#allROIs_sf<-st_sf(ROINumber, geometry=ROI_sfc_combined)

#plot(ROI_sf_combined)
#print(ROI_spat)

#ROI_ppp = as.ppp(ROI_spat, W=square(257))
# type 0 'polygon' ROIs are plotted using lines()
#file <- file.path(system.file(package = "RImageJROI"), "extdata", "ijroi", "polygon.roi")
#x <- read.ijzip(ij_ROIs)
#plot(x, col = "red") 
# Read ROI coordinates from the .csv file
#linecheck = seq(0,256, by=1)