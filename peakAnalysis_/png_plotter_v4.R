...
png_plotter_v4<- function(df = df, png_dir = png_dir,prefix_str=prefix_str, suffix_str = suffix_str,all_lut_str = all_lut_str, dir_regex = dir_regex, vars_to_plot=vars_to_plot,save_bool=save_bool,tag_var=taggy,user_x = user_x, user_y = user_y){
				...
				png_filename = paste0(prefix_str,fixed_vid_key, all_lut_str)
				print(png_filename)  
				...
				######read_png############ so you can change directory to read in Ch2Results.csv
							...
							# 	}
							  ...
							  scalebar = data.frame(x_start = user_x,
								  ...
								  geom_segment(data=scalebar, aes(x=x_start,y=y_start,xend=x_end,yend=y_end), size= 6, colour="white")+
								  
								  labs(x = expression("X ("*mu*"m)"), y = expression("Y ("*mu*"m)"), tag=tag_var) + #paste0(fixed_vid_key,": Spatial Plot with ROI overlay only")) +title = "Z-stack",
								  theme_minimal()+
